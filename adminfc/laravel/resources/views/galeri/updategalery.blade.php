...
@section('maincontent')
    <div class="row">
        <div class="col-md-8 my-3">
            <form action="{{ Route('galery.update', $galery) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="hidden" name="gbr_lama" value="{{ $galery->gambar }}">
                <div class="mb-3 form-group ">
                    <label for="title" class="form-label">Judul galery</label>
                    <input type="text" class="form-control @error('title') is-invalid @endError" id="title"
                        name="judul" required autofocus value="{{ old('judul', $galery->judul) }}">
                    @error('title')
                ...
                <div class="mb-3 form-group ">
                    ...
                    <select class="form-control col-md-4 @error('urutan') is-invalid @endError" id="urutan"
                        name="urutan">
                        @for ($i = 1; $i <= 100; $i++)
                            <option value="{{ $i }}" {{ $i == $galery->urutan ? 'selected' : '' }}>
                                {{ $i }}</option>
                        @endfor
                    </select>
                    @error('urutan')
                ...
                <div class="mb-3 form-group ">
                    ...
                    <select class="form-control col-md-4 @error('aktif') is-invalid @endError" id="urutan"
                        name="aktif">
                        <option value="Y" {{ $galery->aktif == 'Y' ? 'selected' : '' }}>Ya</option>
                        <option value="T" {{ $galery->aktif == 'T' ? 'selected' : '' }}>Tidak</option>
                    </select>
                    @error('aktif')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                ...
                <div class=" form-group">
                    ...
                    @if ($galery->gambar)
                        <img class="img-fluid img-preview col-md-8 mt-3"
                            src="{{ asset('public/images/galeri/' . $galery->gambar) }}">
                    @else
