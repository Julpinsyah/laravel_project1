    ...
    <div class="row">
        <div class="col-md-8 my-3">
            <form action="{{ Route('banner.update', $banner) }}" method="post" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <input type="hidden" name="gbr_lama" value="{{ $banner->gambar }}">
                <div class="mb-3 form-group ">
                    <label for="title" class="form-label">Judul Banner</label>
                    <input type="text" class="form-control @error('title') is-invalid @endError" id="title"
                        name="judul" required autofocus value="{{ old('judul', $banner->judul) }}">
                    @error('title')
                ...
                <div class="mb-3 form-group ">
                    ...
                    <select class="form-control col-md-4 @error('urutan') is-invalid @endError" id="urutan"
                        name="urutan">
                        @for ($i = 1; $i <= 10; $i++)
                            <option value="{{ $i }}" {{ $i == $banner->urutan ? 'selected' : '' }}>
                                {{ $i }}</option>
                        @endfor
                    </select>
                    @error('urutan')
                ...
                <div class="mb-3 form-group ">
                    ...
                    <select class="form-control col-md-4 @error('aktif') is-invalid @endError" id="urutan"
                        name="aktif">
                        <option value="Y" {{ $banner->aktif == 'Y' ? 'selected' : '' }}>Ya</option>
                        <option value="T" {{ $banner->aktif == 'T' ? 'selected' : '' }}>Tidak</option>
                    </select>
                    @error('aktif')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                ...
                <div class=" form-group">
                    ...
                    @if ($banner->gambar)
                        <img class="img-fluid img-preview col-md-8 mt-3"
                            src="{{ asset('public/images/benner/' . $banner->gambar) }}">
                    @else
                ...
                <div class="mb-3 form-group">
                    <input id="keterangan" class="@error('ketrangan') is-invalid @endError" type="hidden" name="keterangan"
                        value="{{ old('keterangan', $banner->keterangan) }}">
                    <label for="konten" class="form-label">Keterangan Banner</label>
                    @error('keterangan')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
