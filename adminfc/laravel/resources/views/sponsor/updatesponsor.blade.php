    ...
    <div class="row">
        <div class="col-md-8 my-3">
            <form action="{{ Route('sponsor.update', $sponsor) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="hidden" name="gbr_lama" value="{{ $sponsor->logo }}">
                <div class="mb-3 form-group ">
                    <label for="perusahaan" class="form-label">Nama Perusahaan</label>
                    <input type="text" class="form-control @error('perusahaan') is-invalid @endError" id="perusahaan"
                        name="perusahaan" required autofocus value="{{ old('perusahaan', $sponsor->perusahaan) }}">
                    @error('perusahaan')
                ...
                <div class="mb-3 form-group ">
                    ...
                    <select class="form-control col-md-4 @error('grup') is-invalid @endError" id="grup" name="grup">
                        <option value="Primary" {{ $sponsor->grup == 'Primary' ? ' selected' : '' }}>Primary</option>
                        <option value="Secondary" {{ $sponsor->grup == 'Secondary' ? ' selected' : '' }}>Secondary</option>
                    </select>
                    @error('grup')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                ...
                <div class="mb-3 form-group ">
                    ...
                    <select class="form-control col-md-4 @error('aktif') is-invalid @endError" id="aktif"
                        name="aktif">
                        <option value="Y" {{ $sponsor->aktif == 'Y' ? ' selected' : '' }}>Ya</option>
                        <option value="T" {{ $sponsor->aktif == 'T' ? ' selected' : '' }}>Tidak</option>
                    </select>
                    @error('aktif')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                ...
                <div class="mb-3 form-group">
                    ...
                    <input type="date" class="form-control @error('tanggal_mulai') is-invalid @endError col-md-4"
                        id="tanggal_mulai" name="tanggal_mulai" required autofocus
                        value="{{ old('tanggal_mulai', $sponsor->tanggal_mulai) }}">
                    @error('tanggal_mulai')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                ...
                <div class="mb-3 form-group ">
                    ...
                    <input type="date" class="form-control @error('tanggal_akhir') is-invalid @endError col-md-4"
                        id="tanggal_akhir" name="tanggal_akhir" required autofocus
                        value="{{ old('tanggal_akhir', $sponsor->tanggal_akhir) }}">
                    @error('tanggal_akhir')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                ...
                <div class=" form-group">
                    ...
                    @if ($sponsor->logo)
                        <img class="img-fluid img-preview col-md-8 mt-3"
                            src="{{ asset('public/images/sponsor/' . $sponsor->logo) }}">
                    @else
