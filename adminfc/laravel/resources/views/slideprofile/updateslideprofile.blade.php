...
@section('maincontent')
    <div class="row">
        <div class="col-md-8 my-3">
            <form action="{{ Route('slideprofile.update', $slideprofile) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="hidden" name="gbr_lama" value="{{ $slideProfile->gambar }}">
                ...
                <div class="mb-3 form-group ">
                    ...
                    <select class="form-control col-md-4 @error('urutan') is-invalid @endError" id="urutan"
                        name="urutan">
                        @for ($i = 1; $i <= 20; $i++)
                            <option value="{{ $i }}" {{ $i == $slideProfile->urutan ? 'selected' : '' }}>
                                {{ $i }}</option>
                        @endfor
                    </select>
                    @error('urutan')
                ...
                <div class="mb-3 form-group ">
                    ...
                    <select class="form-control col-md-4 @error('aktif') is-invalid @endError" id="urutan"
                        name="aktif">
                        <option value="Y" {{ $slideProfile->aktif == 'Y' ? 'selected' : '' }}>Ya</option>
                        <option value="T" {{ $slideProfile->aktif == 'T' ? 'selected' : '' }}>Tidak</option>
                    </select>
                    @error('aktif')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                ...
                <div class=" form-group">
                    ...
                    @enderror
                    @if ($slideProfile->gambar)
                        <img class="img-fluid img-preview col-md-8 mt-3"
                            src="{{ asset('images/profil/' . $slideProfile->gambar) }}">
                    @else
                ...
                <a href="{{ Route('slideprofile.index') }}" class="btn btn-secondary">Kembali</a>
            </form>
        </div>
    </div>
@endsection
