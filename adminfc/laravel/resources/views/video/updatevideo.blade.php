            ...
            <form action="{{ Route('video.update', $video) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="mb-3 form-group ">
                    ...
                    <input type="text" class="form-control @error('judul') is-invalid @endError" id="title"
                        name="judul" required autofocus value="{{ old('judul', $video->judul) }}">
                    @error('title')
                ...
                <div class="mb-3 form-group ">
                    ...
                    <input type="date" class="col-md-4 form-control @error('tanggal') is-invalid @endError"
                        id="title" name="tanggal" required value="{{ old('tanggal', $video->tanggal) }}">
                    @error('tanggal')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                ...
                <div class="mb-3 form-group ">
                    ...
                    <select class="form-control col-md-4 @error('aktif') is-invalid @endError" id="urutan"
                        name="aktif">
                        <option value="Y" {{ $video->aktif == 'Y' ? 'selected' : '' }}>Ya</option>
                        <option value="T" {{ $video->aktif == 'T' ? 'selected' : '' }}>Tidak</option>
                    </select>
                    @error('aktif')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                ...
                <div class="mb-3 form-group ">
                    ...
                    <input type="text" class="form-control @error('link') is-invalid @endError" id="title"
                        name="link" required value="{{ old('link', $video->link) }}">
                    @error('link')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                ...
                <div class="mb-3 form-group ">
                    ...
                    <Textarea type="text" rows="3" class="form-control @error('keterangan') is-invalid @endError" id="keterangan"
                        name="keterangan">{{ old('keterangan', $video->keterangan) }}</Textarea>
                    @error('keterangan')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
