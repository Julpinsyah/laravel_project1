...
@section('maincontent')
    ...
    <div class="row">
        <div class="col-md-8 my-3">
            <form action="{{ Route('pertandingan.update', $tanding) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="mb-3 form-group ">
                    ...
                    <select class="col-md-6 form-control @error('idtanding') is-invalid @endError" id="idtanding"
                        ...
                        @foreach ($liga as $c)
                            <option value="{{ $c->id }}"
                                {{ old('idtanding', $tanding->idtanding) == $c->id ? 'selected' : '' }}>
                ...
                <div class="mb-3 form-group ">
                    ...
                    <select class="form-control @error('tanding') is-invalid @endError" id="tanding" name="tanding"
                        ...
                        @foreach ($club as $c)
                            <option value="{{ $c->id }}"
                                {{ old('tanding', $tanding->tanding) == $c->id ? 'selected' : '' }}>
                                {{ $c->nama }}</option>
                        @endforeach
                    </select>
                    @error('tanding')
                ...
                <div class="mb-3 form-group ">
                    ...
                    <select class="form-control @error('tandang') is-invalid @endError" id="tandang" name="tandang"
                        ...
                        @foreach ($club as $c)
                            <option value="{{ $c->id }}"
                                {{ old('tandang', $tanding->tandang) == $c->id ? 'selected' : '' }}>
                                {{ $c->nama }}</option>
                        @endforeach
                    </select>
                    @error('tandang')
                ...
                <div class="mb-3 form-group ">
                    ...
                    <input type="text" class="form-control @error('lapangan') is-invalid @endError" id="lapangan"
                        name="lapangan" value="{{ old('lapangan', $tanding->lapangan) }}">
                    @error('lapangan')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                ...
                <div class="mb-3 form-group ">
                    ...
                    <input type="text" class="form-control @error('lokasi') is-invalid @endError" id="lokasi"
                        name="lokasi" value="{{ old('lokasi', $tanding->lokasi) }}">
                    @error('lokasi')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                ...
                <div class="mb-3 form-group ">
                    ...
                    <textarea type="text" rows="3" class="form-control @error('map') is-invalid @endError" id="map"
                        name="map">{{ old('map', $tanding->map) }}</textarea>
                    @error('map')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
